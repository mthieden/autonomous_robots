...
*/
            ...
            case ms_init:
                n=2; dist=0.5;angle=5.0/180*M_PI;
                mission.state= ms_fwd;
                break;
            ...
            case ms_follow:
                if (follow_line(1,0.3,mission.time))  {
			printf (  "follow \n ");
			mission.state=ms_end;
			}
                break;
    ...
    if (debug)
    {
        printf("time %05d, x : %f, y : %f, theta : %f, motorspeed_l : %f, motorspeed_r : %f speedcmd: %f, mission stat : %d  motiontype :%d\n", mission.time, odo.x, odo.y, odo.theta,mot.motorspeed_l, mot.motorspeed_r, mot.speedcmd, mission.state, mot.curcmd);
...
void update_motcon(motiontype *p)
{
    ...
    if(p->curcmd == mot_move || p->curcmd == mot_follow_line)
    {
        ...
        {
		update_IR();
            int line_index = lin_pos();
		printf("   line index : %d", line_index);
            double line_com = 0;
            double line_k = 10;
            if (line_index <= -1)
            {
