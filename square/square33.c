...
*/
  ...
  //write log 
   ...
   fp = fopen("/home/smr/k385/NHR/square/log02.dat", "w");  //TO BE EDITED;
   for(int i = 0; i < log_odo_counter; i++){
   	fprintf(fp, "%d, %f, %f\n", log_odo[i].time, log_odo[i].x, log_odo[i].y);
   }
   fclose(fp);
...
void update_odo(odotype *p)
{
  ...
  //update log
   ...
   printf("time %d, x : %f, y : %f, theta : %f\n", mission.time, odo.x, odo.y,odo.theta);
   log_odo_counter++;
}
...
if (p->cmd !=0){
   ...
   //printf("time %d, left : %f, right : %f\n", mission.time, mot.motorspeed_l, mot.motorspeed_r);
